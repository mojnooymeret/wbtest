...
namespace OzonWb
{
   internal class Program
   {
      ...
      static void Main()
      {
         ...
         //   //if (urls[i].price == "0") countNull++;
         //   //else countYes++;
         //   if (urls[i].price == "0" && urls[i].service == "ozon") countNull++;
         //   else if (urls[i].price == "0" && urls[i].service == "wildberries") countYes++;
         //}
         test();
      ...
      public static EdgeOptions GetOptions()
      {
         try {
            ...
            //options.AddArguments(new List<string>() { "--no-sandbox", "--disable-dev-shm-usage" });
            return options;
         } catch { return null; }
      }
   }
