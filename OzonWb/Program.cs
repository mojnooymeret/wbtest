...
namespace OzonWb
{
   internal class Program
   {
      ...
      static int timeOzon = 0;
      static int timeWildberries = 0;
      private static async void sumTime()
      {
         try {
            while (true) {
               timeOzon++;
         ...
         try {
            ...
            for (int i = 0; i < data.Count; i++) {
               ...
               try {                                                                               // добавляется рандомный user-agent
                  ...
                  if (result == "ng= ₽" || result.Contains("<") || result.Contains(">") || result.Contains("=") || result.Contains("meta"))
